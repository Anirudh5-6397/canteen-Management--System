...
header('location:index.php');
  ...
  {	
	...
	$sql="insert into feedback (sender, title, reciver, feedbackdata) values (:user,:title,:reciver,:description)";
	$query = $dbh->prepare($sql);
	$query-> bindParam(':user', $sender, PDO::PARAM_STR);
	$query-> bindParam(':title', $replytitle, PDO::PARAM_STR);
	$query-> bindParam(':reciver', $replyto, PDO::PARAM_STR);
...
<div class="form-group">
	...
	<div class="col-sm-4">
	<input type="text" name="title" class="form-control" readonly required value="<?php echo htmlentities($replytitle);?>">
	</div>
</div>
